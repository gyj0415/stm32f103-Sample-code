...
/*如果你不想使用这里面(_I2C_.h)的函数，想使用自己的IIC写寄存器函数,则可以把你的写寄存器函数替换掉
#define MPU6050Wirte_Reg(reg,data) 你的写寄存器函数
#define MPU6050Read_Reg(reg)       你的读寄存器函数
*/
#include <stdbool.h>
...
void MPU6050_Get_Angle(MPU6050* this){
	...
	this->yaw += Gz/2 + 0.0005;//减小零飘(如果零票严重，可以通过加减一个小数来解决,这是二次手动校准,值得注意的是，每次上电的时候gyro_zero_z的值都会不同，并且VCC也会变换所以这个不是很好的解决办法，不想漂就加磁力计)
}
...
void MPU6050_Get_Angle_Plus(MPU6050* this) {
    ...
    float _twoKp;
    float _twoKi;
    const float halfT = 0.01f;   // 采样时间10ms，根据实际情况调整(F103没有FPU，计算浮点数很慢)

    float recipNorm;
    float qDot1, qDot2, qDot3, qDot4;
    static float _integralFBx = 0.0f, _integralFBy = 0.0f, _integralFBz = 0.0f;//积分量

    // 从寄存器里读出角度，并减去校准后的零偏值
    ...
    q_x += qDot2 * halfT;
    q_y += qDot3 * halfT;
    q_z += qDot4 * halfT + 0.00003f;//可以用来手动减小yaw零点漂移,适当+-后面的数

    // 归一化四元数
    recipNorm = 1.0f / sqrt(q_w * q_w + q_x * q_x + q_y * q_y + q_z * q_z);
    if (isnan(recipNorm) ||(recipNorm != 0.0f / 0.0f)) {
    ...
    // 将弧度转换为角度
    this->roll = atan2f(2 * (q_w * q_x + q_y * q_z), 1 - 2 * (q_x * q_x + q_y * q_y)) * 57.29578f -2.3;////在后面+-角度可以让传感器刚开机就是0
    this->pitch = asinf(2 * (q_w * q_y - q_z * q_x)) * 57.29578f + 5;//在后面+-角度可以让传感器刚开机就是0
